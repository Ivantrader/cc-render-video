// server.js — Render Video API (corrigido)
...
async function synthTTS(segments = [], voice = "BR-M1") {
  // Placeholder: silêncio (pronto para plugar TTS real depois)
  const total =
...
app.post("/render_video", async (req, res) => {
  try {
    // Aceita body como objeto ou string JSON
    let body = req.body;
    ...
    // timeline também pode vir como string
    ...
    // Normaliza formato
    const fmt =
      String(format).includes("9:16") || String(format).includes("9x16")
        ? "9:16"
        : "16:9";
    const W = fmt === "9:16" ? 1080 : 1920;
    const H = fmt === "9:16" ? 1920 : 1080;

    // Defaults de timeline
    const fps = Number(timeline.fps || 30);
    const duration =
      Number(timeline.duration) ||
      (String(body?.duration || "").endsWith("s")
        ? Number(String(body.duration).replace("s", "")) || 45
        : 45);
    const tracks = timeline.tracks || {};

    // 1) Voz (silêncio por enquanto)
    const voicePath = await synthTTS(
      tracks.voiceover || [],
      tracks.voiceover?.[0]?.voice || "BR-M1"
    );
    // 2) Vídeo
    ...
    if (mode === "preview") {
      ...
      const urlPrev = await savePublic(
        pv,
        `preview_10s_${fmt.replace(":", "x")}.mp4`,
        req
      );
    ...
    const finalUrl = await savePublic(
      withAudio,
      `video_final_${fmt.replace(":", "x")}_${duration}s.mp4`,
      req
    );
    ...
    res
      .status(500)
      .json({ error: "render_failed", detail: String(e?.message || e) });
  }
});
